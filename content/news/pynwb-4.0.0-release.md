...
In practice, you can attach and retrieve a HERD instance directly from your file using the new helper:

```python
herd = nwbfile.get_external_resources()
```

This creates a HERD instance for the file if one does not already exist, or returns the existing one. From there you can register external references against the objects and values in your file. By bringing HERD into the core `NWBFile` interface, PyNWB makes ontology-grounded annotation a first-class part of the NWB workflow.
